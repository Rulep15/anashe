...
$pdf->SetFillColor(255, 255, 255);
    ...
    if (!empty($compras)) {
        foreach ($compras as $compra) {
            ...
            $pdf->Cell(50, 5, 'NRO FACTURA', 0, 0, 'C', 1);
            $pdf->Cell(50, 5, 'CONDICION', 0, 0, 'C', 1);
            $pdf->Cell(40, 5, 'CANTIDAD COMPRA', 0, 0, 'C', 1);
            $pdf->Cell(40, 5, 'IVA TOTAL', 0, 0, 'C', 1);
            ...
            $pdf->SetFillColor(255, 255, 255);
            $pdf->SetFont('', '', 10);
            $pdf->Cell(15, 5, $compra['id_compra'], 0, 0, 'C', 1);
            $pdf->Cell(50, 5, $compra['fechac'], 0, 0, 'C', 1);
            $pdf->Cell(50, 5, $compra['prv_razon_social'], 0, 0, 'C', 1);
            $pdf->Cell(50, 5, $compra['nro_factura'], 0, 0, 'C', 1);
            $pdf->Cell(50, 5, $compra['condicion'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $compra['cant_cuo'], 0, 0, 'C', 1);
            $pdf->Cell(40, 5, $compra['ivatotal'], 0, 0, 'C', 1);
            ...
            $detalles = consultas::get_datos("select * from v_compras_detalle where id_compra=" . $compra['id_compra'] . " order by id_compra");

            $pdf->SetFont('', 'B', 10);
            $pdf->SetFillColor(188, 188, 188);
            $pdf->Cell(100, 5, 'ARTICULO', 1, 0, 'C', 1);
            $pdf->Cell(100, 5, 'PRECIO UNIT', 1, 0, 'C', 1);
            $pdf->Cell(100, 5, 'CANTIDAD', 1, 0, 'C', 1);
            $pdf->Ln(); //salto
            ...
            foreach ($detalles as $detalle) {
                ...
                $pdf->Cell(100, 5, $detalle['cantidad'], 1, 0, 'C', 1);
                $pdf->Ln();
            }
            $pdf->Ln();
            $pdf->Cell(350, 0, '----------------------------------------------------------------------------------------------------------------------------------------'
