...
$pdf->Cell(0, 0, "Reporte de Pedidos", 0, 1, 'C');
$pdf->Ln();
...
if ($_REQUEST['vopcion'] == '2') {
    $sqls = consultas::get_datos("SELECT * FROM v_compras_pedido WHERE id_pedido !=0 AND estado=" . $_REQUEST['vciudad'] . "ORDER BY id_pedido");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
