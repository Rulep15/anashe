...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_compras WHERE com_fecha BETWEEN $fechainicial AND $fechafinal ");
if (!empty($pedidoss)) {
    foreach ($pedidoss as $pedidos) {
        ...
        $pdf->Cell(290, 3, 'DETALLE DE COMPRA NRO.:  ' . $pedidos['id_compra'], 0, 0, 'C', 0);
        $pdf->Ln();
        $pdf->Ln();

        $detalles = consultas::get_datos("select * from v_detalle_compras where id_compra=" . $pedidos['id_compra']);
