...
$pdf->SetTitle('REPORTE DE LIBROS DE COMPRAS');
...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_libros WHERE com_fecha BETWEEN $fechainicial AND $fechafinal ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['id_libro'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            ...
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['com_fecha'], 0, 0, 'L');
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->SetFont('', 'B', 14);
            ...
            $pdf->Cell(290, 3, 'DETALLE DE LIBRO NRO.:  ' . $pedidos['id_libro'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_libros where id_libro=" . $pedidos['id_libro']);
            if (!empty($detalles)) {
                $pdf->SetFont('', 'B', 13);
