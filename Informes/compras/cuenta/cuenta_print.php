...
$pdf->SetTitle('REPORTE DE CUENTAS');
...
$pdf->AddPage('L', 'LEGAL');
//celda para titulo
$pdf->Cell(0, 0, "REPORTE DE CUENTAS", 0, 1, 'C');
...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_cuentas WHERE cta_vto BETWEEN $fechainicial AND $fechafinal ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['id_cuenta'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(22, 7, 'Nro Fact:', 0, 0, 'C');
            $pdf->Cell(30, 7, $pedidos['com_nro_factura'], 0, 0, 'C');
            ...
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['cta_vto'], 0, 0, 'L');
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->SetFont('', 'B', 14);
            ...
            $pdf->Cell(290, 3, 'DETALLE DE CUENTA NRO.:  ' . $pedidos['id_cuenta'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_cuentas where id_cuenta=" . $pedidos['id_cuenta']);
            if (!empty($detalles)) {
                $pdf->SetFont('', 'B', 13);
