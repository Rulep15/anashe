...
$pdf->SetTitle('Reporte de Nota Debito');
...
if ($_REQUEST['vopcion'] == '1') {
    $sqls = consultas::get_datos("SELECT * FROM v_nota_debito WHERE id_debito=" . $_REQUEST['vciudad'] . "ORDER BY id_debito");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['id_pedido'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['fecha_pedido'], 1, 0, 'C', 1);
            $pdf->Cell(70, 5, $sql['usu_nick'], 1, 0, 'C', 1);
            $pdf->Cell(100, 5, $sql['descri'], 1, 0, 'C', 1);
...
if ($_REQUEST['vopcion'] == '2') {
    $sqls = consultas::get_datos("SELECT * FROM v_compras_pedido WHERE estado=" . $_REQUEST['vciudad'] . "ORDER BY id_pedido");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['id_pedido'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['fecha_pedido'], 1, 0, 'C', 1);
