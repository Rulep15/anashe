...
$pdf->Cell(0, 0, "Reporte de Presupuesto", 0, 1, 'C');
$pdf->Ln();
...
if ($_REQUEST['vopcion'] == '2') {
    $sqls = consultas::get_datos("SELECT * FROM v_presupuesto WHERE id_presu !=0 AND estado=" . $_REQUEST['vciudad'] . "ORDER BY id_presu");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['id_presu'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['fecha_pedido'], 1, 0, 'C', 1);
            $pdf->Cell(70, 5, $sql['prv_razon_social'], 1, 0, 'C', 1);
