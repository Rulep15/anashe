...
$pdf->AddPage('L', 'LEGAL');
//celda para titulo
$pdf->Cell(0, 0, "REPORTE PRESUPUESTO", 0, 1, 'C');
...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_presupuesto WHERE fecha BETWEEN $fechainicial AND $fechafinal ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['id_presu'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(22, 7, 'USUARIO:', 0, 0, 'C');
            $pdf->Cell(30, 7, $pedidos['empleado'], 0, 0, 'C');
            ...
            $pdf->Cell(290, 3, 'DETALLE DE PRESUPUESTO NRO.:  ' . $pedidos['id_presu'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();
