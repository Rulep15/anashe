...
$pdf->Ln();
$pdf->Cell(60, 5, "N° Orden", 1, 0, 'C', 1);
$pdf->Cell(50, 5, "N° Presupuesto", 1, 0, 'C', 1);
...
if ($_REQUEST['vopcion'] == '1') {
    $sqls = consultas::get_datos("SELECT * FROM v_orden_de_compra WHERE nro_orden=" . $_REQUEST['vciudad'] . "ORDER BY nro_orden");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['nro_orden'], 1, 0, 'C', 1);
            if ($sql['id_presu'] == 0) {
                $pdf->Cell(50, 5, 'VACIO', 1, 0, 'C', 1);
            } else {
                $pdf->Cell(50, 5, $sql['id_presu'], 1, 0, 'C', 1);
            }
...
if ($_REQUEST['vopcion'] == '2') {
    $sqls = consultas::get_datos("SELECT * FROM v_orden_de_compra WHERE estado=" . $_REQUEST['vciudad'] . "ORDER BY nro_orden");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['nro_orden'], 1, 0, 'C', 1);
            if ($sql['id_presu'] == 0) {
