...
if ($_REQUEST['vopcion'] == '1') {
    ...
    $pedidoss = consultas::get_datos("SELECT * FROM v_orden_de_compra WHERE fecha BETWEEN $fechainicial AND $fechafinal ");
    if (!empty($pedidoss)) {
        foreach ($pedidoss as $pedidos) {
            ...
            $pdf->Cell(20, 7, $pedidos['nro_orden'], 0, 0, 'L');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            $pdf->Cell(50, 7, '', 0, 0, 'C');
            ...
            $pdf->Ln();
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->Cell(20, 7, 'N° Presu: ', 0, 0, 'L');
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['id_presu'], 0, 0, 'L');
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Ln();
            ...
            $pdf->SetFont('', 'N', 14);
            $pdf->Cell(30, 7, $pedidos['fecha_orden'], 0, 0, 'L');
            $pdf->Ln();
            $pdf->Ln();
            $pdf->Cell(20, 7, '', 0, 0, 'C');
            $pdf->SetFont('', 'B', 14);
            ...
            $pdf->Cell(290, 3, 'DETALLE DE ORDEN DE COMPRA NRO.:  ' . $pedidos['nro_orden'], 0, 0, 'C', 0);
            $pdf->Ln();
            $pdf->Ln();

            $detalles = consultas::get_datos("select * from v_orden_detalle where nro_orden=" . $pedidos['nro_orden']);
            if (!empty($detalles)) {
                $pdf->SetFont('', 'B', 13);
