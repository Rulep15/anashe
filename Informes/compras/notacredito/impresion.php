...
if ($_REQUEST['vopcion'] == '2') {
    $sqls = consultas::get_datos("SELECT * FROM v_nota_de_credito WHERE estado=" . $_REQUEST['vciudad'] . "ORDER BY cod_notc");
    if (!empty($sqls)) {
        foreach ($sqls AS $sql) {
            $pdf->Cell(60, 5, $sql['cod_notc'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['btc_fecha_sistema'], 1, 0, 'C', 1);
            $pdf->Cell(60, 5, $sql['usu_nick'], 1, 0, 'C', 1);
            $pdf->Cell(60, 5, $sql['prv_razon_social'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['estado'], 1, 0, 'C', 1);
            $pdf->Cell(50, 5, $sql['descripcion'], 1, 0, 'C', 1);
            $pdf->Ln();
        }
    } else {
